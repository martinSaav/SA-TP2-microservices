...
import com.sa.apirest.clinic.exception.ClinicMismatchException;
import com.sa.apirest.clinic.exception.ClinicNotFoundException;
import com.sa.apirest.clinic.exception.NotValidBodyException;
import com.sa.apirest.clinic.exception.UnknownErrorException;
import com.sa.apirest.clinic.interfaces.BaseController;
import com.sa.apirest.clinic.model.Base;
import com.sa.apirest.clinic.service.BaseServiceImpl;
...
public abstract class BaseControllerImpl<E extends Base, S extends BaseServiceImpl<E, Integer>> implements BaseController<E, Integer> {
    ...
    public ResponseEntity<?> getRecordById(@PathVariable Integer id) {
        try {
            ...
            throw new ClinicNotFoundException(e.getMessage(), HttpStatus.NOT_FOUND);
        } catch (Exception e) {
            throw new UnknownErrorException(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<?> update(@PathVariable(required = false) Integer id, @RequestBody E entity) {
        try {
            ...
            throw new ClinicMismatchException(e.getMessage(), HttpStatus.BAD_REQUEST);
        } catch (DataIntegrityViolationException e) {
            throw new NotValidBodyException(e.getMessage(), HttpStatus.BAD_REQUEST);
        } catch (Exception e) {
            throw new UnknownErrorException(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
